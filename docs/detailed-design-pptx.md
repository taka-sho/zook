...
### 8.8 リンク経路の重複検知(実装時に追加)

要求：矢印(コネクタ)が経路の途中で無関係な要素やテキストと重なっていることを機械的に検出したい。

- コネクタの実際の接続点座標は §8.2 のマッピング(`connection_point()`)で決定的に計算できるため、`style: straight` の場合はこの2点を結ぶ線分がそのままレンダリング結果と一致する。この線分と、始点・終点の要素本体・祖先・子孫を除く全要素の footprint 矩形、および他リンクのラベル矩形との交差判定(線分と矩形の交差、標準的な向き判定アルゴリズム)で「経路上の重なり」を機械的に検出する。
- `elbow`/`curved` は実際の折れ線・曲線の形状を python-pptx 側のプリセットジオメトリが決めるため、この直線近似では完全には再現できない。始点・終点を結ぶ直線で近似判定するため、参考値(見逃し・誤検知ともにあり得る)として扱う。
