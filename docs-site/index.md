...
| 衝突の自動解消 | `zook doctor` で3段階に自動解消: 要素の重なりを座標調整、リンクのノード貫通・見かけ上の直接接続を接続辺の割り当て、迂回できない貫通は障害物要素の退避(いずれも悪化しない範囲のみ)。既定はドライラン(提案のみ)、`--fix`/`-o` で YAML に書き戻し([使い方](usage.md)の doctor 節) |
