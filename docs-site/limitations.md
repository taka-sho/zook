...
なお、これらの多くは `zook doctor` で自動解消できます([使い方](usage.md)の doctor 節参照)。**兄弟要素どうし・要素とコンテナ見出しの重なり**は要素をずらして解消し、**リンクのノード貫通・リンクラベルの衝突**は接続辺(fromSide/toSide)の割り当てで解消を試みます(悪化しない範囲でのみ変更)。ただし接続辺の変更では経路を障害物の外まで動かせないケース(例: 垂直に並んだ2要素のちょうど間にある障害物)は解消できず、その場合は `remaining` として報告されるだけなので、生成後の手直しが必要です。
